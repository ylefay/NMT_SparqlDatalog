import json
from utils.utils import datalog_preprocessing, sparql_preprocessing
#Construct the data.{en,datalog} file.. used by nmt from json.

#Create the data files.
def create_data_files(DATASET_PATH, file_paths, json_db):
    with open(DATASET_PATH+file_paths[0], 'w+') as file_en, open(DATASET_PATH+file_paths[1], 'w+') as file_datalog, open(DATASET_PATH+file_paths[2], 'w+') as file_sparql:
        possible_english_labels = ["intermediary_question", "corrected_question", "question"]
        s = json_db[0]    
...
def create_data_files_tags(DATASET_PATH, file_paths, json_db):
    ...
    with open(DATASET_PATH+file_paths[0], 'w+') as file_bert, open(DATASET_PATH+file_paths[1], 'w+') as file_br:
        for s in json_db:
