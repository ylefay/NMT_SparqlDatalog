...
if __name__ == '__main__':
    ...
    requiredNamed.add_argument('--dataset', dest='dataset',
                               metavar='dataset.sparql', help='sparql or datalog dataset file', required=True)
    requiredNamed.add_argument('--srcextension', dest='srcextension', help='en for example', required=True)
    requiredNamed.add_argument('--tgtextension', dest='tgtextension', help='datalog or sparql for example', required=True)
    ...
    with io.open(tgt_file, encoding="utf-8") as original_tgt, io.open(src_file, encoding="utf-8") as original_src:
        ...
        with io.open(f'train.{tgtextension}', 'w', encoding="utf-8") as train_tgt, io.open(f'train.{srcextension}', 'w', encoding="utf-8") as train_src, \
                io.open(f'dev.{tgtextension}', 'w', encoding="utf-8") as dev_tgt, io.open(f'dev.{srcextension}', 'w', encoding="utf-8") as dev_src, \
                io.open(f'test.{tgtextension}', 'w', encoding="utf-8") as test_tgt, io.open(f'test.{srcextension}', 'w', encoding="utf-8") as test_src:
