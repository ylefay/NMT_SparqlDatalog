...
def full_pipeline(ce_untagged_query, silent=False):
    ...
    os.system(f'./ask.sh ./trained_models/LC-QuAD_bert_br "{bert_pos_tags}" bert br > tmp')
    br_tags_file = open('./tmp', 'r').read()
    ...
    os.system(f'./ask.sh ./trained_models/LC-QuAD_en_datalog "{kb_simplified_query}" en datalog > tmp')
    datalog_file = open('./tmp', 'r').read()
    os.remove('./tmp')
    datalog_query = (datalog_file.split('\n')[-3])

    #Replace back 
    mapping_replace = {mapping_replace[key]:do_replacements(key,{' ':'_'}) for key in mapping_replace.keys()}
    ...
    for idx, s in enumerate(json_db):
        try:
            out_json[idx] = {
                ...
                'datalog_prev': full_pipeline(drop_brackets(s[english_label]), silent=True)
            }
            out_json[idx].update({'dist':levenshtein(out_json[idx]['datalog_prev'], out_json[idx]['datalog_query'])})
        except:
...
if __name__ == "__main__":
    ...
    json_db = json_db[501:min(len(json_db), N)]
