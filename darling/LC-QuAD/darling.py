...
def darling_run(sparql_queries_iter_index):
    for idx in sparql_queries_iter_index:
        if (
            ...
            file.close()
            os.system(
                f"cp ./inputs/ontology--DEV_type\=parsed.ttl ./inputs/ontology--DEV_type\=parsed_{idx}.ttl"
            )
            os.system(
                f"perl darling.pl -q ./inputs/queries_{idx}.sparql -t ./inputs/ontology--DEV_type\=parsed_{idx}.ttl"
            )
            os.system(
                f"mv ./ontology--DEV_type=parsed_{idx}_query_1.asp ./outputs/_query_{idx}.asp"
            )
...
if __name__ == "__main__":
    ...
    processes = [
        Process(
            target=darling_run,
            args=([k + N_cpu * i for i in range(len(sparql_txt) // N_cpu)],),
        )
