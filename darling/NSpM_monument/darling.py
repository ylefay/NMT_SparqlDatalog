...
ontology_name = (
    "Monument_ontology.ttl"  # excepted localisation: ./inputs/{ontology_name}
)
...
def darling_run(sparql_queries_iter_index, ontology_name):
    for idx in sparql_queries_iter_index:
        if (
            ...
            os.system(
                f"cp ./inputs/{ontology_name} ./inputs/{ontology_name[:-3]}_{idx}.ttl"
            )
            os.system(
                f"perl darling.pl -q ./inputs/queries_{idx}.sparql -t ./inputs/{ontology_name[:-3]}_{idx}.ttl"
            )
            os.system(
                f"mv ./{ontology_name[:-3]}_{idx}_query_1.asp ./outputs/_query_{idx}.asp"
            )
            ...
            if os.path.exists(f"./{ontology_name[:-3]}.asp"):
                ...
                os.system(
                    f"mv ./{ontology_name[:-3]}_{idx}.asp ./{ontology_name[:-3]}.asp"
                )
