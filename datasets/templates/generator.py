...
def sort_matches(matches, template):
    ...
    matches_with_usages = [
        {"usages": get_usages(match), "match": match} for match in matches
    ]
...
def generate_dataset(templates, output_dir, file_mode):
    ...
    it = 0
    with io.open(
        output_dir + "/data.en", file_mode, encoding="utf-8"
    ) as english_questions, io.open(
        output_dir + "/data.sparql", file_mode, encoding="utf-8"
    ) as sparql_queries:
        for template in tqdm(templates):
            ...
            try:
                ...
                # print bindings
                if bindings is None:
                    id_or_question = getattr(template, "id") or getattr(
                        template, "question"
                    )
                ...
                for binding in bindings:
                    ...
                    if dataset_pair:
                        ...
                        b = re.search(
                            "brack_open(.*)brack_close", dataset_pair["sparql"]
                        )
                ...
                logging.error(
                    "template {} caused exception {}".format(
...
def prepare_generator_query(
    template, add_type_requirements=True, do_special_class_replacement=True
):
    generator_query = getattr(template, "generator_query")
    ...
    for i, variable in enumerate(variables):
        ...
        if variable_has_a_type and add_type_requirements:
            ...
            if variable_is_subclass(generator_query, variable):
                generator_query = add_requirement(
                    generator_query,
                    SUBCLASS_REPLACEMENT.format(
                        variable=variable, ontology_class=normalized_target_class
                    ),
                )
            else:
                if (
                    normalized_target_class in SPECIAL_CLASSES
                    and do_special_class_replacement
                ):
                    classes = " ".join(
                        [
                            "({})".format(c)
                    ...
                    generator_query = add_requirement(
                        generator_query,
                        CLASSES_REPLACEMENT.format(variable=variable, classes=classes),
                    )
                else:
                    ontology_class = normalized_target_class
                    generator_query = add_requirement(
                        generator_query,
                        CLASS_REPLACEMENT.format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--continue",
        dest="continue_generation",
        action="store_true",
        help="Continue after exception",
    )
    ...
    used_resources = (
        collections.Counter(json.loads(open(resource_dump_file).read()))
        if use_resources_dump
