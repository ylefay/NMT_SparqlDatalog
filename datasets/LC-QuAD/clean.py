...
def fix_1():
    for i, json_db in enumerate(dbs):
        for idx, s in enumerate(json_db):
            if s['intermediary_question'].count('<')==1 and s['intermediary_question'].count('>')==1 and ('What is the' in s['intermediary_question'] or 'Who is the' in s['intermediary_question'] or 'Who are the' in s['intermediary_question'] or 'What are the' in s['intermediary_question']) :
...
if __name__ == '__main__':
    ...
    with open("./test-data.json", "w+") as out_test, open("./train-data.json", "w+") as out_train:
